...
function InhRef() {
  return (
    <span className="inherited-ref" title="inherited reference">
      &
    </span>
  );
}
...
const prettyOptions = {
  ...
  inherited_ref_on_ref: {
    EatOuter: (
      <>
        &p:{" "}
        <Underline title="reference to consider">
          <InhRef />
        </Underline>
        &T
      </>
    ...
    EatInner: (
      <>
        &p: <InhRef />
        <Underline title="reference to consider">&</Underline>T
      </>
    ),
    EatBoth: (
      <>
  ...
  fallback_to_outer: {
    ...
    EatOuter: (
      <>
        &mut p:{" "}
        <Underline title="reference to consume">
          <InhMutRef />
        </Underline>{" "}
        &T
      </>
...
export function Preset({ options, setOptions }) {
  ...
  const active_bundle = bundlesDoc
    .filter((bundle) => bundle.name == active_bundle_name)
    .pop();
  ...
  const bundles = bundlesDoc.map((bundle) => {
    return (
      <option key={bundle.name} value={bundle.name} title={bundle.doc}>
    ...
    <OverlayTrigger
      ...
      overlay={
        <Tooltip>
          {active_bundle
            ? active_bundle.doc
            : "This ruleset does not have a name"}
        </Tooltip>
      }
    >
      <Form.Select
...
export function OptionElem({ option, options, setOptions, fullWidth }) {
  ...
  const variant = disabled
    ? "outline-light"
    : current_val == "false" || current_val == "Error" || current_val == "No"
    ? "outline-danger"
    : "outline-success";

  function textForValue(value_name) {
    return (
      prettyOption[value_name] ||
  ...
  return (
    <OverlayTrigger
      placement="bottom"
      overlay={<Tooltip>{current_val_doc}</Tooltip>}
    >
      <Button
        variant={variant}
        ...
        disabled={disabled}
        onClick={(_e) => {
          setKey(option.name, next_value.name);
          return false;
        }}
      >
        {hiddenOptionsForWidth}
        <span style={style}>{textForValue(current_val)}</span>
      </Button>
    </OverlayTrigger>
  );
}
  ...
  const select_solver = (
    ...
    >
      ...
      >
        <CalculateWidth
          contents={"Ty"}
          setWidth={(w) => setSolverBtnWidth((old_w) => Math.max(old_w, w))}
        />
        <CalculateWidth
          contents={"BM"}
          setWidth={(w) => setSolverBtnWidth((old_w) => Math.max(old_w, w))}
        />
        <span
          style={{ display: "inline-block", width: solverBtnWidth || null }}
        >
    ...
    <Navbar expand="xl" className="bg-body-tertiary">
      <Container fluid>
        <Stack direction="horizontal" gap={1}>
          {title ? (
            <Navbar.Brand className="text-nowrap me-3">{title}</Navbar.Brand>
          ) : null}
          {select_solver}
        ...
        >
          ...
          <Offcanvas.Body>
            <Nav>
              <Stack
                direction={navShow ? "vertical" : "horizontal"}
                gap={1}
                className={navShow ? "" : "ms-1"}
              >
                {option_elems}
              </Stack>
            </Nav>
